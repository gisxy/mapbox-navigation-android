...
import com.mapbox.navigation.navigator.toNavigatorSensorData
import com.mapbox.navigation.navigator.toSensorType
...
class SensorFileWriter private constructor(
    ...
    suspend fun write(sensorList: List<Sensor>) = withContext(Dispatchers.IO) {
        ...
        file.outputStream()
            .use { fos ->
                ...
                sensorList.onEach {
                    val sensorType = it.toSensorType()?.name
    ...
    suspend fun write(sensorEvent: SensorEvent) = withContext(Dispatchers.IO) {
        sensorEvent.toNavigatorSensorData()
            ?.mapToEventRow()
            ?.let { eventRow ->
